...
{
    public partial class Popup_Kyougou : Form
    {
        ...
        private void get_data()
        {
            ...
            using (var conn = new SqlConnection(connStr))
            {
                ...
                cmd.CommandText = "SELECT " +
                    ...
                    "WHERE KyougouNarabijun <> 0 ";

                if (src_1.Text != "")
                {
                    cmd.CommandText += " AND KyougouKigyouCD COLLATE Japanese_XJIS_100_CI_AS_SC LIKE N'%" + GlobalMethod.ChangeSqlText(src_1.Text, 1) + "%' ESCAPE'\\' ";
                }
                if (src_2.Text != "")
                {
                    cmd.CommandText += " AND KyougouMeishou COLLATE Japanese_XJIS_100_CI_AS_SC LIKE N'%" + GlobalMethod.ChangeSqlText(src_2.Text, 1) + "%' ESCAPE'\\' ";
                }
                if (src_3.Text != "")
                {
                    cmd.CommandText += " AND KyougouFurigana COLLATE Japanese_XJIS_100_CI_AS_SC LIKE N'%" + GlobalMethod.ChangeSqlText(src_3.Text, 1) + "%' ESCAPE'\\' ";
                }
                if (src_4.Text != "")
                {
                    cmd.CommandText += " AND KyougouHonshaJuusho COLLATE Japanese_XJIS_100_CI_AS_SC LIKE N'%" + GlobalMethod.ChangeSqlText(src_4.Text, 1) + "%' ESCAPE'\\' ";
                }


                cmd.CommandText += " ORDER BY KyougouNarabijun ";
        ...
        private void c1FlexGrid1_BeforeMouseDown(object sender, C1.Win.C1FlexGrid.BeforeMouseDownEventArgs e)
        {
            ...
            if (hti.Column == 0 & hti.Row > 0)
            {
                ...
                ReturnValue[2] = c1FlexGrid1.Rows[_row][5].ToString();
                this.Close();
            }
        }
